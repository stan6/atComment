...
public class ForwardGenerator extends AbstractGenerator {
  ...
  public ExecutableSequence step() {
    ...
    if (GenInputsAbstract.force_null && NullForcer.forceNullFromPrev) {  
      NullForcer.moveToNextNullForcedSequence(executionVisitor);
      eSeq = processNullForcedSequence();
    } else {
 ...
 // TODO I think this was used for the first approach at forcing null
 //
   /*
    ExecutableSequence nullSeq = createNewUniqueSequence(false);
    if(nullSeq!=null){
      nullSeq.execute(executionVisitor);
      //check if the nullSeq cause any failure, if yes, 
      //append to the original seq(original seq also shouldn't have failure)
      if(!eSeq.hasFailure() && nullSeq.hasCheck(DocCheck.class) && nullSeq.hasFailure()){
        int failureInd = nullSeq.getFailureIndex();
        // Figure out input variables.
	List<Variable> inputs = nullSeq.sequence.getInputs(failureInd);
	List<Sequence> seqs = new ArrayList<Sequence>();
        seqs.add(eSeq.sequence);
        seqs.add(nullSeq.sequence);
	Sequence newSeq = Sequence.concatenate(seqs);
	eSeq = new ExecutableSequence(newSeq);
	System.out.println("Con:"+eSeq.sequence);
        //eSeq.extend.(nullSeq.sequence.getStatementKind(failureInd), inputs);
      }
    }*/
  ...
  private ExecutableSequence createNewUniqueSequence() {
    ...
    // If there is an inference for this statement, we'll make sure selectInputs() doesn't gives us
    // any null inputs at first and that they're forced later.  When this is false, selectInputs()
    // will behave normally with respect to generating null inputs
    boolean hasNullInference = false;
    
    if (GenInputsAbstract.force_null) {
      hasNullInference = InferNullUtils.getKnownInferences(statement).size() > 0;
    }

    // jhp: add flags here
    InputsAndSuccessFlag sequences = selectInputs(statement, hasNullInference);
    
// TODO I think this was used for the first approach at forcing null
    //
    // for increasing the sequence with null inputs
//   InputsAndSuccessFlag  nullSeq = selectInputs(statement,true);
...
//    int index=0;
//    boolean match=false;
//    while(index < sequences.sequences.size() && index < nullSeq.sequences.size()){
//		if(!sequences.sequences.get(index).equals(nullSeq.sequences.get(index)))
//			break;
//		else
//			match = true;
//       index++;
//    }
//    if(!match)
//    	sequences.sequences.addAll(nullSeq.sequences);
    ...
    // This will be 0 unless there's a repeat heuristic
    NullForcer.repeatLast = 0;
...
// TODO I think this was used for the first approach at forcing null
//
//    for (Sequence is : nullSeq.sequences) {
//      subsumed_sequences.add (is);
//    }
       
    // If there is an inference for this statement, keep track of it and
    // start forcing null parameter permutations
    if (GenInputsAbstract.force_null) {
      if (hasNullInference) {
        NullForcer.startForcingNull(newSequence);
      }
    }
  ...
  protected ExecutableSequence processNullForcedSequence() {
    Sequence newSequence = NullForcer.getCurrentSequence();
    
    // TODO Is this really necessary for forced-null sequences?  If so, I'm not sure yet how to
    // generate the InputsAndSuccessFlags for it...
    // InputsAndSuccessFlag sequences = NullForcer.getCurrentInputsAndSuccessFlag();

    // If sequence is larger than size limit, try again.
    if (newSequence!=null && newSequence.size() > GenInputsAbstract.maxsize) {
      if (Log.isLoggingOn()) Log.logLine("Sequence discarded because size " + newSequence.size() + " exceeds maximum allowed size " + GenInputsAbstract.maxsize);
        return null;
    }
  ...
  private InputsAndSuccessFlag selectInputs(StatementKind statement,boolean hasNullInference) {
...
// TODO I think this was used for the first approach at forcing null
//        
//        HashMap<Integer, NullStatus> nullStatuses=InferNullUtils.getNullStatusesForStatement(statement);
//        String inferenceString ="";
//        if (nullStatuses.keySet().size() > 0) {
...
//          NullStatus status = nullStatuses.get(i);
//  
...
//              inferenceString += "Specific-" + exceptionName;
//            }
//          }
//          //}
//         // System.out.println("Inference string:"+inferenceString);
//          //System.out.println("statement:"+statement);
//          //System.out.println("----------------------");
//        }
//
//        if (GenInputsAbstract.force_null && !inferenceString.isEmpty() && !inferenceString.contains("Unknown") && use_null) {
...
// TODO I think this was used for the first approach at forcing null
//             
//        else if(!GenInputsAbstract.force_null){
//          if (Log.isLoggingOn()) Log.logLine("Will use null as " + i + "-th input");
//          StatementKind st = PrimitiveOrStringOrNullDecl.nullOrZeroDecl(t);
//          Sequence seq = new Sequence().extend(st, new ArrayList<Variable>());
//          variables.add(totStatements);
...
//        }
      ...
      if (!hasNullInference // Only use null ratio if these inputs will not later be forced by NullForcer
                            // i.e., if it doesn't have null-inferences associated with it
          && !isReceiver
